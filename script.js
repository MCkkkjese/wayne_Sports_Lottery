...
const orderPanel = document.querySelector('#order-section');
...
function openPanel(panelName) {
	const targetPanel = panelName === 'order' ? orderPanel : cartPanel;
	const otherPanel = panelName === 'order' ? cartPanel : orderPanel;
...
function closePanels() {
	[orderPanel, cartPanel].forEach((panel) => {
		if (!panel) {
			return;
		}
				...
				<details class="reward-tier ${unlocked ? 'is-unlocked' : ''}">
					<summary>
						<div class="reward-tier__header">
							...
							<div class="reward-tier__meta">
								<div class="reward-tier__threshold">${tier.threshold} 積分</div>
								<span class="reward-tier__icon">▾</span>
							</div>
						</div>
					</summary>
					<div class="reward-grid">
						${tier.items.map((item) => `<div class="reward-item"><div class="reward-item__label">${item}</div><div class="reward-item__amount">${tier.threshold} 積分</div></div>`).join('')}
					</div>
				</details>
			`;
		})
